...
openssl req -new -newkey rsa:4096 -nodes -keyout client.key -subj /CN=localhost/ -passin pass:secret -passout pass:secret > client.csr
openssl x509 -req -CA rootCA.crt -CAkey rootCA.key -in client.csr -out client.crt -days 365 -CAcreateserial -passin pass:secret
openssl pkcs12 -export -out client.p12 -name “client” -inkey client.key -in client.crt -passin pass:secret -passout pass:secret

mkdir -p ~/.postgresql
cp -f rootCA.crt ~/.postgresql/root.crt
cp -f client.crt ~/.postgresql/postgresql.crt
cp -f client.key ~/.postgresql/postgresql.key
